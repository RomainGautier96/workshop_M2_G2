...
function lister_branches_php_min() {
	return array(
		'master' => '5.6.0',
		'3.2' => '5.4.0',
		'3.1' => '5.1.0',
 ...
 */
define('_SPIP_LOADER_VERSION', '4.2.0');
...
function lister_branches_proposees($branche = null) {
	...
	if ($branches === null) {
		$branches = array();
		$liste = lister_versions_spip();
		$php_min_branches = lister_branches_php_min();
		foreach ($liste as $path => $desc) {
			$br = $desc['branche'];
			$php_min = isset($php_min_branches[$br]) ? $php_min_branches[$br] : $php_min_branches['master'];
			$branches[$br] = $desc;
			$branches[$br]['php'] = $php_min;
		}
	}
	if (!is_null($branche)) {
...
function lister_versions_spip() {
	...
	if (
		...
		$contenu = spip_loader_recuperer_page(_URL_SPIP_LOADER_LIST);
		if ($contenu) {
			ecrire_fichierT($local_spip_loader_list, $contenu);
		} else {
			throw new \Exception("Impossible de récupérer le fichier " . _URL_SPIP_LOADER_LIST);
		}
	}
	...
	$api = $liste['api'];
	if (!$api or $api > 1) {
		throw new \Exception("Version d’API $filename innatendue. Une mise à jour de spip_loader s’impose !");
	}

	$liste = $liste['versions'];
	$versions = array();
	foreach ($liste as $version => $path) {
		$versions[$path] = array(
			'version' => $version,
			'branche' => ($version === 'dev' ? 'dev' : getBranchNameFromTag($version)),
...
function getBranchNameFromTag($tag) {
	if (!$tag) {
		return "";
	}
	if ($tag[0] === 'v') {
 ...
 * @return string
 */
function menu_branches($active, $version_installee, $presente) {
	$select = '';
...
function spip_presente_deballe($fichier, $paquet, $dest, $range) {
	...
	if (
		...
		$ma_branche = getBranchNameFromTag($version_installee);
		$branches = lister_branches_proposees();
	...
	$corps =
		...
		. menu_branches($paquet, $version_installee, $presente)
		. '</div>'
		. '</div>'
		. $bouton;
